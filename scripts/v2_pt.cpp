...
void v2_no_eff_correction(const char* rawFileName, const char* outFileName, float y_cut, float pT_lo_nq, float pT_hi_nq, const char* EPD_method, bool use_2D, bool use_mT=false, int sys_tag=0)
{   
    ...
    ofstream resFile(outFileNameStr.replace(outFileNameStr.find("_cen1.csv"), 4, "_res.csv"));
    ...
    float v2_pt[num_par][num_EP][num_cen][200] = {0};
    float v2_pt_err[num_par][num_EP][num_cen][200] = {0};
    float v2_pt_counts[num_par][num_cen][200] = {0};
    ...
    // each centrality as a file
    for (int cen=0; cen<num_cen; cen++)
    {
        std::string outFileNameStr_cen(outFileName);
        outFileNameStr_cen.replace(outFileNameStr_cen.find("_cen1.csv"), 4, Form("_cen%d.csv", cen+1));
        ofstream outputFile(outFileNameStr_cen);
        ...
        for (int ptbin=0; ptbin<200; ptbin++)
        {
            for (int j=0; j<num_par; j++) outputFile << v2_pt_counts[j][cen][ptbin] << ",";
            for (int j=0; j<num_par; j++) outputFile << v2_pt[j][0][cen][ptbin] << ",";
            for (int j=0; j<num_par; j++) outputFile << v2_pt_err[j][0][cen][ptbin] << ",";
            for (int j=0; j<num_par; j++) outputFile << v2_pt[j][1][cen][ptbin] << ",";
            for (int j=0; j<num_par-1; j++) outputFile << v2_pt_err[j][1][cen][ptbin] << ",";
            outputFile << v2_pt_err[num_par-1][1][cen][ptbin] << endl;
        }
        outputFile.close();
    }
    f->Close();
