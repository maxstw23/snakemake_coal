...
def percentage_sigma(scale_1, scale_2, sigma_1, sigma_2, opt=3):
    ...
    while cur_perc < perc[opt]:
        ...
        if perc[opt] - cur_perc > 0.05:
            ...
            ind_left -= 1
            ind_right += 1
        area_signal = _trapezoid(y[ind_left:ind_right], dx=0.2 / num_div)
...
class PolyMinvFit:
    ...
    def __init__(self, path, par_str, poly_deg, guesses, monitor=True, **kwargs):
        ...
        hist = file[f'h{self.par_str}M_pt_1_cen_1']
        ...
        if 'ybin' in kwargs.keys() and 'cen' in kwargs.keys():
            ...
            if self.ptbin == [1]:
                self.ratio = 1
            y_vals = np.sum(np.array([file[f'h{self.par_str}M_pt_{j}_cen_{i}'].values()
                                        for i in self.cen for j in self.ybin]), axis=0)
            y_errs = np.sum(np.array([file[f'h{self.par_str}M_pt_{j}_cen_{i}'].errors()
                                        for i in self.cen for j in self.ybin]) ** 2, axis=0) ** 0.5
            v2_hists = [
                file[f"h{self.par_str}_{self.EP}_v2_pt_{j}_cen_{i}"] for i in self.cen for j in self.ybin]
    ...
    def fit_v2(self, masked=False, v2=0.05, a=0, b=0, save_to=None, cen=None, ybin=None):
        ...
        if self.monitor or save_to is not None:
            ...
            if save_to is not None and cen is not None and ybin is not None:
                ...
                cen_string = save_to.split('/')[-1].split('_')[-2]
                y_string = save_to.split('/')[-1].split('_')[-1].split('.yaml')[0]
                save_to = save_to.replace(cen_string, f'cen{cen[0]}') #.replace(y_string, f'y{y_lb:.1f}{y_rb:.1f}')
                # create path if needed
                path = Path(save_to.replace('invmass', 'v1fit')).parent
                path.mkdir(parents=True, exist_ok=True)
                with open(save_to.replace('invmass', 'v1fit'), 'w') as f:
...
def main(path, outputDir, particle_str, EP, yrebin=1, max_refit=500, paper_plot_path=None, **kwargs):
    start = time.time()
    monitor = 0
    paper_plot_cen = 4
    paper_plot_y = 2
    ...
    # if no lambda in file, output empty csv
    try:
        file_test = uproot.open(path)
        hist_test = file_test[f'hLambdaM_pt_1_cen_1']
    except uproot.exceptions.KeyInFileError:
        print('No Lambda found in', path)
        # output empty csv
        dict_empty = {}
        df_empty = pd.DataFrame(dict_empty)
        df_empty = df_empty.stack().apply(pd.Series)
        df_empty.index = df_empty.index.set_names(['First_Level', 'Second_Level'])
        df_empty = df_empty.sort_index(level=[1, 0], ascending=[True, False])
        df_empty = df_empty.swaplevel().T
        print(f'Writing to {outputDir}')
        df_empty.to_csv(outputDir)
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--particle', type=str, default=None,
                        help='particle type, e.g. Lambda, Xi, Omega')
    parser.add_argument('--EP', type=str, default='EPD',
                        help='event plane type, e.g. TPC, EPD')
    ...
    # particle_str = inputDir.split('/')[-1].split('_')[1]
    # flow_case = inputDir.split('/')[-1].split('_')[-2]
    result = outputDir.split('/')[0] + '/'
    if args.debug:
        kw_dict = {'cen': args.cen_bin, 'ybin': args.y_bin}
    else:
        kw_dict = {}
    main(inputDir, outputDir, args.particle, args.EP,
